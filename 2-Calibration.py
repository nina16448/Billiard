...
margeY = 300
l_point_test = []
for x in [margeX, WIDTH_MAX - margeX]:
    for y in [margeY, HEIGHT_MAX - margeY]:
        l_point_test += [[x, y]]
...
try:
    ...
    l_circle_screen = [[0, 0], [0, HEIGHT_MAX], [WIDTH_MAX, 0], [WIDTH_MAX, HEIGHT_MAX]]
...
def draw_circle(event, x, y, flags, param):
    ...
    if event in [cv2.EVENT_RBUTTONDOWN, cv2.EVENT_LBUTTONDOWN]:
        if len(l_circle) >= 4:
            l_circle[
                min(
                    enumerate([(xC - x) ** 2 + (yC - y) ** 2 for xC, yC in l_circle]),
                    key=lambda x: x[1],
                )[0]
            ] = [x, y]
        else:
...
calibration_test = np.zeros((HEIGHT_MAX, WIDTH_MAX, 3), np.uint8)
...
def ordering(l_point1, l_point2):
    ...
    for l_ordering in permutations(list(range(len(l_point1)))):
        ...
        for i1, i2 in enumerate(l_ordering):
            d += np.linalg.norm(np.array(l_point1[i1]) - np.array(l_point2[i2]))
...
calibration_test2 = np.zeros((HEIGHT_MAX, WIDTH_MAX, 3), np.uint8)
...
test_1 = cv2.warpPerspective(
    calibration_test2, tMat1, (WIDTH_MAX, HEIGHT_MAX), flags=cv2.INTER_LINEAR
)
cv2.putText(
    test_1,
...
test_2 = cv2.warpPerspective(
    frame_camera, tMat2, (WIDTH_MAX, HEIGHT_MAX), flags=cv2.INTER_LINEAR
)
cv2.putText(
    test_2,
