...
# body.position = (0, 0)
# # lEdges = [
#     pymunk.Segment(body, (0, 0), (cadre[0], 0), 10),
...
class Ball:
    ...
    #                 * v
    #                 * 2
    #                 * float(np.exp(-self.debut_simulation / 4))
    #             )
    ...
    def add_pos(self, t, x, y):
        ...
        #     self.lVitesse_prediction += [
        #         [t_physic_engine]
        #         + list(self.body.velocity)
...
while True:
    ...
    for ball in Ball.lBall:
        ...
        if flagg == False and t - clean_time > 3 and v == 0:
            if put_time == 0:
                put_time = t
            if t - put_time > 5:
        ...
        #         pts = np.array(ball.lChemin + ball.lVt1[::-1], np.int32).reshape(
        #             (-1, 1, 2)
        #         )
        ...
        #         prediction = np.array(
        #             [[[x, y]] for t, x, y in ball.lPos_prediction], np.int32
        #         )
    ...
    # newframe = cv2.warpPerspective(
    #     newframe, m_projector2camera, (1920, 1080), flags=cv2.INTER_LINEAR
    # )
    ...
    if idBall in [b.id for b in Ball.lBall]:
        ...
        axs[0, 0].set(
            xlabel="Time (s)", ylabel="Speed (px/s)", title="Speed of the ball"
        )
        ...
        axs[1, 0].set(xlabel="X", ylabel="Y", title="Position of the ball")
